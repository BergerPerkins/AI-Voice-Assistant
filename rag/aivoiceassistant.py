...
class AIVoiceAssistant:
    ...
    def __init__(self):
        ...
        self._llm = Ollama(model="mistral", request_timeout=500.0)
        self.embed_model = HuggingFaceEmbedding("sentence-transformers/all-MiniLM-L6-v2")
        self._service_context = ServiceContext.from_defaults(llm=self._llm, embed_model=self.embed_model)
    ...
    def _create_kb(self):
        try:
            reader = SimpleDirectoryReader(
                input_files=[r"D:\Work\C_Drive\voice_chatbot\voice_assistant_llm\rag\restaurentfile.txt"]
            )
            ...
            print(f"Timeout error while creating knowledge base: {e}")
            raise ValueError("Failed to create knowledge base due to timeout")
